...
- Web 工作台同步来源与版本治理：缺失/部分来源可通过手填 URL 或显式授权 GitHub 搜索补全；候选需用户确认后保存；新增缓存/强制联网版本检查、来源溯源展示、过期/分叉状态和实例级 `update --dry-run` 预览。所有写入与联网操作均受本机同源专用 API 和请求校验保护。
- 新增来源发现：缺失来源时可手动填写 URL，或在明确授权后通过 GitHub 官方 API 搜索并验证候选 `SKILL.md`；候选必须由用户选择才会保存，并记录 manual/search/install-url/frontmatter、置信度和验证时间等来源元数据。
- `skm scan` 默认只使用 24 小时内的本地更新缓存；显式 `skm scan --online` 才联网刷新已登记来源，并在扫描结果与治理总览中提示过期/分叉 skill 及实例级 dry-run 更新命令。检查失败或缺少来源时明确报告结果不完整，不再误报全部最新。
