...
function summarizeTools(catalog) {
  const tools = ['claude-code', 'codex', 'cursor', 'gemini', 'workbuddy', 'kimi', 'pi'];
...
function commandCatalog(lang) {
  ...
  return [
    ...
    { id: "list", group: "explore", icon: "list", command: "skm list",
      ...
      mode: "read", executable: true,
      params: [
        { flag: "--tool", label: en ? "tool" : "工具", type: "value", values: ["claude", "codex", "cursor", "gemini", "workbuddy", "kimi", "pi"], hint: en ? "Filter by client tool" : "按客户端工具筛选" },
    ...
    { id: "install", group: "lifecycle", icon: "plus", command: "skm install <source> --dry-run",
      ...
      params: [
        ...
        { flag: "--tool", label: en ? "tool" : "工具", type: "value", values: ["claude", "codex", "cursor", "gemini", "workbuddy", "kimi", "pi"], hint: en ? "Target client tool" : "目标客户端" },
        { flag: "--dry-run", label: "dry-run", type: "bool", hint: en ? "Preview plan only" : "仅预览计划" },
