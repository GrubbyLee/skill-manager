...
test('表格：彩色单元格按可见宽度截断并补重置码，普通方括号文本不受影响', () => {
  const ESC = String.fromCharCode(27);
  const red = (s) => `${ESC}[31m${s}${ESC}[0m`;
  const t = truncate(red('中文很长的描述文本内容'), 10);
  assert.ok(displayWidth(t) <= 10, '彩色串可见宽度超限');
  assert.ok(t.endsWith(`${ESC}[0m`), '截断后未补颜色重置码');
  assert.equal(displayWidth('[已知局限] abc'), 14);
  assert.ok(truncate('[已知局限] 某个很长的标题文字', 12).startsWith('[已知局限]'));
});
...
test('相对时间：按日历日计算（昨晚 23:30 在今晨查看应为"昨天"），脏值回退', () => {
  const now = Date.parse('2026-07-19T08:00:00+08:00');
  assert.equal(fmtAgo('2026-07-18T23:30:00+08:00', now), '昨天');
  assert.equal(fmtAgo('2026-07-19T01:00:00+08:00', now), '今天');
  assert.equal(fmtAgo('2026-07-16T12:00:00+08:00', now), '3 天前');
  assert.equal(fmtAgo('2026-05-10T12:00:00+08:00', now), '2 个月前');
  assert.equal(fmtAgo('2025-07-01T00:00:00+08:00', now), '1 年前');
